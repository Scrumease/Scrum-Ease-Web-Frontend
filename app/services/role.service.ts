...
import { RoleDocument } from "../interfaces/role/role.document";

export interface RoleService {
  getAll: (filter: FilterInterface) => Promise<FindPaginated<RoleDocument>>;
...
export const roleService: RoleService = {
  getAll: async (filter) => {
    try {
      const response = await api.get<FindPaginated<RoleDocument>>(basePath, {
        params: { ...filter },
      });
